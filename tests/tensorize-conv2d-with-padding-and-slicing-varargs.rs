...
fn conv2d_im2col_tensorize_to_smaller_array_with_padding_and_slicing_varargs() {
    ...
    pub const IMAGE_SHAPE: &[usize] = &[1, 56, 56, 64];
    pub const KERNEL_SHAPE: &[usize] = &[3, 3, 64, 64];
    ...
    map.insert("image".to_string(), IMAGE_SHAPE.to_vec());
    // kernel height, kernel width, in channels, out channels
    map.insert("weights".to_string(), KERNEL_SHAPE.to_vec());
    ...
    let base_rws = vec![
        ...
        glenside::language::rewrites::slice_concatenate_accesses(
            0,
            SliceConcatenateStrategy::DivideIntoVarargs { segment_size: 64 },
        ),
        glenside::language::rewrites::slice_concatenate_accesses(
            1,
            SliceConcatenateStrategy::DivideIntoVarargs { segment_size: 64 },
