...
pub fn simplify_access_flattens() -> Rewrite<Language, MyAnalysis> {
    rewrite!(
        "simplify-access-flattens";
        "(access-flatten (access-flatten ?a))" =>
        "(access-flatten ?a)"
    )
}

pub fn simplify_access_reshapes() -> Rewrite<Language, MyAnalysis> {
    rewrite!(
        "simplify-access-reshapes";
        "(access-reshape (access-reshape ?a ?unused) ?shape)" =>
...
"
    ...
    #[test]
    fn simplify_access_flattens() {
        let mut map = HashMap::default();
        map.insert("a".to_string(), vec![32, 64, 3, 2]);
        let program = "
         (access-flatten (access-flatten (access-tensor a)))
        "
        .parse()
        .unwrap();
        let mut egraph =
        ...
        let rws = vec![super::simplify_access_flattens()];
        ...
        let matches = "
          (access-flatten (access-tensor a))
            "
        .parse::<Pattern<_>>()
        .unwrap()
        .search_eclass(&runner.egraph, id)
    ...
    fn simplify_access_reshapes() {
        ...
        let rws = vec![super::simplify_access_reshapes()];
